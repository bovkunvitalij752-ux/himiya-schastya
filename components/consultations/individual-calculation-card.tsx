...
const worldviewMeanings: Record<number, string> = {
  1: "мир воспринимается через действие: важно видеть цель, направление и возможность влиять на ситуацию",
  2: "мир воспринимается через атмосферу: тон, безопасность и эмоциональный климат значат очень много",
  3: "мир воспринимается через обмен: разговор, признание, вдохновение и ощущение живого интереса",
  4: "мир воспринимается через структуру: спокойнее, когда есть план, правила и понятные обязанности",
  5: "мир воспринимается через свободу: важно не застревать, иметь выбор и пространство для движения",
  6: "мир воспринимается через любовь: дом, близость, красота и забота становятся главным языком души",
  7: "мир воспринимается через смысл: хочется докопаться до сути и не жить поверхностно",
  8: "мир воспринимается через результат: важны эффективность, развитие, признание и ощущение силы",
  9: "мир воспринимается через мудрость: важны человечность, польза, принятие и большой горизонт",
};
      ...
      {result ? (
        <div className="mt-5 rounded-[1.5rem] border border-[#ead9be] bg-[#fffaf2] p-4">
          ...
          <div className="mt-4 grid gap-3">
            ...
            <InsightBlock title="Внутренний мир" text={result.worldviewText} />
...
function calculateIndividualCard(name: string, birthDate: string): IndividualResult {
  ...
  return {
    ...
    worldviewText: `Внутренний мир ${worldview}: ${worldviewMeanings[worldview]}. Это помогает понять, что человеку важно для чувства безопасности, доверия и настоящей близости.`,
